...
const int num_chromosome = 50;
const int num_parents = 30;
const int range = 800;
...
void satisfied_vec_func_offspring(chromosome &x) { //satisfied vector calculation for offsprings
	for (int i = 0; i < num_node; i++)
	{
		x.satisfied[i] = 0;
	}
...
void prob_func(population &gen) { // Calculating probabilities of chromosomes ***Note: Min Fitness Prob=0***
	for (int i = 0; i < num_chromosome; i++) {
		if (gen.chros[i].fitness < minimum) {
			minimum = gen.chros[i].fitness;
		}
		else if (gen.chros[i].fitness > maximum)
		{
			maximum = gen.chros[i].fitness;
		}
	}
	for (int i = 0; i < num_chromosome; i++) {
		gen.chros[i].prob = (gen.chros[i].fitness - minimum) / (maximum - minimum);
	}
	float sum = 0;
	for (int i = 0; i < num_chromosome; i++)
	{
...
chromosome satisfied_new(chromosome x) { //satisfied vector calculation
	for (int i = 0; i < num_node; i++)
	{
		x.satisfied[i] = 0;
	}
		...
		for (int iter = 0; iter < num_iter; iter++)
		{
			...
			for (int i = 0; i < num_chromosome; i++)
			{
				...
				for (int j = 0; j < num_node; j++)
				{
					...
					///if(initial.chros[i].satisfied[j]==1)
                    ///{
                    ///    cout <<" ";
                    ///    cout << Array_node[j].demand;
                    ///    cout <<" ";
                    ///}
			...
			*/
			...
			{
				satisfied_vec_func_offspring(offsprings[i]);
			...
			*/
			for (int i = 0; i < num_chromosome; i++) //Calculating satisfied vector for new population
			{
				satisfied_vec_func(initial.chros[i]);
			}

			for (int j = 0; j < num_chromosome; j++) //Calculate fitness values of new population
			{
				initial.chros[j].fitness = fitness_func(initial.chros[j]);
			}
			...
			*/
			...
			if (iter == num_iter - 1) {
				cout << "END";
				int stop_s = clock();
				timer = (stop_s - start_s) / double(CLOCKS_PER_SEC) * 1000;
